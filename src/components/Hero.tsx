...
export default function Hero({ dict }: { dict: Dictionary['hero'] }) {
  ...
  return (
    <section className="min-h-[90vh] flex items-center relative overflow-hidden hero-atmosphere pt-24 lg:pt-0">
      <div className="max-w-content mx-auto px-6 lg:px-8 w-full">
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          >
            ...
            <h1 className="text-[38px] sm:text-[52px] lg:text-[64px] font-semibold leading-[0.98] tracking-tight text-text-primary mb-5">
              {dict.title.split('\n').map((line, i) => (
            ...
            <div className="flex flex-wrap gap-2">
              {dict.tags.map((tag, i) => {
                ...
                return (
                  <span
                    key={tag}
                    className="px-3 py-1.5 text-sm font-medium rounded-lg border border-transparent"
                    style={{ color: colors[i % 3], background: softs[i % 3] }}
                  >
                    {tag}
                  </span>
                );
